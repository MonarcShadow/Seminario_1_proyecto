"""
Script de entrenamiento con TRPO para progresión de herramientas.

TRPO (Trust Region Policy Optimization) ofrece garantías teóricas de mejora
monotónica de la política mediante restricción de KL divergence.
...
Referencia: Schulman et al. (2015) - "Trust Region Policy Optimization"
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train TRPO agent with curriculum learning')
    ...
    parser.add_argument('--cg-max-steps', type=int, default=15,
                       help='Conjugate gradient max steps (default: 15)')
...
def train():
    ...
    run_name = f"trpo_curriculum_{timestamp}" if args.curriculum else f"trpo_single_{timestamp}"
    
    print("\n" + "="*70)
    print("TRPO TRAINING - TOOL PROGRESSION")
    print("="*70)
    ...
    if args.resume:
        ...
        model = TRPO.load(args.resume, env=env)
        ...
        if curriculum and curriculum.current_stage_idx > 0:
            ...
            if pretrained_path:
                ...
                try:
                    model = TRPO.load(pretrained_path, env=env)
                    model.set_logger(logger)
