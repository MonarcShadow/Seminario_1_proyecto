"""
Experiment Runner - Stage 5 (From Scratch)
Ejecuta todos los algoritmos secuencialmente para completar el tech tree completo.
...
def run_experiment(algorithm, episodes=50, env_seed=123456, port=10000, load_model=True):
    ...
    - load_model: si True, carga el modelo de diamond por defecto
    """
    print(f"\n{'='*70}")
    print(f"EJECUTANDO: {algorithm.upper()} - Stage 5 (From Scratch)")
    print(f"{'='*70}")
    ...
    if load_model:
        ...
        if os.path.exists(diamond_model_path):
            ...
            print(f"✓ Cargando modelo pre-entrenado: {diamond_model_path}")
        else:
            print(f"⚠ Warning: No se encontró modelo de diamond para {algorithm}, entrenando desde cero")
...
def main():
    ...
    print("\n" + "="*70)
    print("EXPERIMENTO COMPLETO - Stage 5 (From Scratch - Complete Pipeline)")
    print("="*70)
